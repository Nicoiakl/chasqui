...
test('V1 · expires <= deliver_after se rechaza al enviar (el sobre vencería antes de llegar)', async () => {
  ...
  try {
    ...
    await assert.rejects(() => a.send({ to: `agentea@${dom}`, body: 'x', deliverAfter: cuando, expires: antes }), /vencería antes|deliver_after/);
  } finally { await e.stop(); }
});
...
test('V1 · un sobre que vence esperando en la cola REBOTA al remitente, no desaparece', async () => {
  ...
  try {
    ...
    const env = { nyx5: '1', id: crypto.randomUUID(), from: `agentea@${dom}`, to: [`x@${dom}`],
    ...
    await e.store.enqueue({ id: crypto.randomUUID(), envelope: env, domain: dom, to: [`x@${dom}`],
      from_local: "agentea", attempts: 1, next_attempt: pasado, created: pasado, status: 'queued', log: [] });
    await e.tick();
